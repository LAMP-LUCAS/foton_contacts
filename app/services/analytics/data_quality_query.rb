module Analytics
  class DataQualityQuery
    ...
    def calculate
      total_contacts = Contact.person.count.to_f
      return {} if total_contacts.zero?

      outdated_count = Contact.person.where('updated_at < ?', 1.year.ago).count
      ...
      {
        orphan_contacts: Contact.person.left_outer_joins(:contact_issue_links, :employments_as_person).where(contact_issue_links: { id: nil }, contact_employments: { id: nil }).distinct.count,
